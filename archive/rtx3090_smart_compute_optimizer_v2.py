...
class SmartGPUComputeOptimizerV2:
    ...
    def _detect_gpu_configuration(self):
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                logger.info(f"🔍 Auto-detected GPU configuration: {configs}")
                return configs
            else:
                logger.warning("⚠️ No CUDA GPUs detected, using CPU fallback")
                return [{"gpu_type": "RTX3070", "count": 1}]  # Will fail gracefully in worker
        except Exception as e:
            logger.warning(f"⚠️ GPU detection failed: {e}, using default config")
            return [{"gpu_type": "RTX3070", "count": 1}]
        
    def run_smart_optimization(self, duration_minutes: int):
        """Run smart GPU compute optimization"""
        logger.info("🧠 RTX 3090 + RTX 3070 SMART COMPUTE OPTIMIZER V2")
        logger.info("=" * 70)
        ...
        logger.info("  ✅ Auto-GPU detection and optimization")
        logger.info("=" * 70)
    ...
    def _display_smart_results(self, results: List[Dict], total_time: float):
        ...
        logger.info("=" * 70)
        logger.info("🎯 SMART GPU COMPUTE OPTIMIZATION RESULTS V2")
        logger.info("=" * 70)
